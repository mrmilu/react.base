...
import { fromJson } from "@/src/common/utils/class-transformer";

@injectable()
export class DummyRepository implements IPostsRepository {
  ...
  async createPost(input: CreatePostInput): Promise<Post | null> {
    ...
    return response?.createPost ? fromJson(PostDataModel, response.createPost).toDomain() : null;
  }
  ...
  async posts(): Promise<Array<Post>> {
    ...
    return dataPostList.map((dataPost) => fromJson(PostDataModel, dataPost).toDomain());
  }
}
